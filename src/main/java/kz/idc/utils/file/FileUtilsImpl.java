...
public final class FileUtilsImpl implements FileUtils {
    ...
    public InputStream getPlan() {
        try {
            File[] files = getDirectory(PLAN.PATH).listFiles();
    ...
    private static String getRootPath() {
        try {
            ...
            String createPath = Application.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
            String[] pathArr = createPath.split("/");
            for (String s : pathArr) {
                if (!s.contains(".jar") && !s.isBlank()) {
